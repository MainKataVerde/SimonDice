...
public class Main {
	...
	public static void play() {
		...
		mostrarSecuencia(MAX_COLORES_SEQ, secuenciaColores);
		...
		if (comprobarColor(0, charColores(letra), secuenciaColores)==false) {
			i++;
			while (i<secuenciaColores.length) {
				System.out.print("Introduce el siguiente color : ");
				letra = scanner.next().charAt(0);
				if (comprobarColor(i, charColores(letra), secuenciaColores)==false) {
					i++;
				}else {
					i = secuenciaColores.length;
